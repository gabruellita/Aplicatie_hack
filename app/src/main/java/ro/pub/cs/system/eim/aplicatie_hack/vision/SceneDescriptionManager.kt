...
class SceneDescriptionManager(private val context: Context) {
    ...
    fun describe(bitmap: Bitmap) {
        ...
        scope.launch {
            val result = callGeminiWithRetry(bitmap)
            withContext(Dispatchers.Main) {
                when (result) {
                    is GeminiResult.Success -> {
                        broadcastMessage(result.text, isError = false)
                        speak(result.text)
                    }
     ...
     */
    private suspend fun callGeminiWithRetry(bitmap: Bitmap, attempt: Int = 0): GeminiResult {
        val apiKey = BuildConfig.GEMINI_API_KEY
        if (apiKey.isNullOrBlank()) {
            return GeminiResult.Error("GEMINI_API_KEY lipsă din local.properties")
        }

        val bodyJson = buildRequestJson(encodeImage(bitmap))
            ...
            response.use { resp ->
                when {
                    resp.code == 429 -> {
                        ...
                        delay(waitMs)
                        callGeminiWithRetry(bitmap, attempt + 1)
                    }
